...
def build_transformation(cfg, split):
    ...
    if split == "train":
        ...
        if cfg.transforms.get('random_crop') is not None:
            t.append(transforms.RandomCrop(cfg.transforms.random_crop.crop_size))

        if cfg.transforms.get('random_horizontal_flip') is not None:
            t.append(
                transforms.RandomHorizontalFlip(p=cfg.transforms.random_horizontal_flip)
            )

        if cfg.transforms.get('random_affine') is not None:
            t.append(
        ...
        if cfg.transforms.get('color_jitter') is not None:
            t.append(
    ...
    if cfg.transforms.get('norm') is not None:
